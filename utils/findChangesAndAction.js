...
const notion = new Client({ auth: process.env.NOTION_TOKEN });
const databaseId = process.env.NOTION_DATABASE_ID_BY_API;
...
async function findChangesAndAction() {
  ...
  for (const [pageId, page] of Object.entries(currItemInDatabase)) {
    if (!itemInDatabase[pageId]) {
      itemInDatabase[pageId] = page;
      sendMessageToSpace(`${page.place}(이/가) 추가됐습니다`);
    } else if (page.status !== itemInDatabase[pageId].status) {
      itemInDatabase[pageId] = {
        status: page.status,
      };
      console.log('status change');
      sendMessageToSpace(`${page.place} status changed to ${page.status}`);
    }
  }
...
async function getItemFromDatabase() {
  ...
  do {
    ...
    const { results, next_cursor } = await notion.request(requestPayload);
